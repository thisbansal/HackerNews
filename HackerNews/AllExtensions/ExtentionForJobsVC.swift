...
extension JobsViewController: UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        ...
        if let title = getArticle(at: indexPath.row)?.title{
            var bodyLabelSize: CGSize = CGSize(width: 0, height: 0)
            let titleLabelSize =  getEstimatedHeightAndWidthOfCell(fontSize: 16.0, nameOfFont: "Avenir-Medium", labelString: title)
            if let domain      = getArticle(at: indexPath.row)?.domain {
                bodyLabelSize  = getEstimatedHeightAndWidthOfCell(fontSize: 11.0, nameOfFont: "Menlo-Italic", labelString: domain)
            } else if let user = getArticle(at: indexPath.row)?.user {
                bodyLabelSize  = getEstimatedHeightAndWidthOfCell(fontSize: 11.0, nameOfFont: "Menlo-Italic", labelString: user)
            } else if let type = getArticle(at: indexPath.row)?.type {
                bodyLabelSize  = getEstimatedHeightAndWidthOfCell(fontSize: 11.0, nameOfFont: "Menlo-Italic", labelString: type)
            }
